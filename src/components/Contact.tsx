...
export default function Contact({ onSubmitRequirement, activeRequirements, preselectedService }: ContactProps) {
  ...
  React.useEffect(() => {
    if (preselectedService) {
      setRoleType(preselectedService);
      setRequirementsSummary(`Seeking professional assistance for: ${preselectedService}. Please contact us to map our talent acquisition framework...`);
    }
  }, [preselectedService]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Build mailto link with all user-entered details
    const mailtoUrl = `mailto:contact@nvaraa.com?subject=${encodeURIComponent(
      `New Hiring Inquiry - ${contactName} ${companyName ? `(${companyName})` : ''}`
    )}&body=${encodeURIComponent(
      `Hello Nvaraa Solutions,

Here are the details from the contact form submission:

- Name: ${contactName}
- Company: ${companyName || 'Not Specified'}
- Work Email: ${email}
- Phone: ${phone}
- Hiring Requirement: ${roleType}
          ...
          <div className="lg:col-span-7 bg-white p-8 rounded-2xl border border-slate-150 shadow-sm">
            <div className="flex gap-3 items-center mb-6 pb-4 border-b border-slate-100">
              ...
              <div>
                ...
                <p className="font-sans text-xs text-slate-500 mt-0.5">Tell us about your hiring plans.</p>
              </div>
            </div>
            ...
            {justSubmitted ? (
              <div className="text-center py-10 space-y-4 animate-fade-in">
                ...
                <p className="font-sans text-sm text-slate-600 max-w-sm mx-auto leading-relaxed">
                  Thank you! We have received your inquiry. A hiring partner from Nvaraa will reach out to you within 24 hours.
                </p>
                <button
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                {/* Hiring Requirement (text box) */}
                <div>
                  <label className="block font-sans font-semibold text-xs text-slate-700 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                    <Sliders className="w-3.5 h-3.5 text-slate-400" />
                    Hiring Requirement <span className="text-rose-500">*</span>
                  </label>
                  <input
                    type="text"
                    required
                    placeholder="E.g. Technical Architect, Growth Marketer, Founding Engineer"
                    value={roleType}
                ...
                <div>
                  ...
                  <textarea
                    rows={4}
                    required
                    placeholder="Provide details about your talent assessment needs, hiring plan, or other inquiries..."
